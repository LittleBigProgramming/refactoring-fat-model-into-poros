...
class Stylist < ActiveRecord::Base
  ...
  validates_each :name do |model, attr, value|
    ...
    if !stylist.nil? && (stylist.id != model.id)
  ...
  def save_services(lengths, prices)
    ...
    salon.services.active.each_with_index do |service, i|
      next unless lengths[i].to_i > 0 || prices[i].to_i > 0
  ...
  def formatted_rent
    if rent > 0
      number_with_precision(rent, precision: 2)
    else
      ''
    end
  end
  ...
  def formatted_service_commission_rate
    if service_commission_rate > 0
      number_with_precision(service_commission_rate, precision: 2)
    else
      ''
    end
  end
  ...
  def formatted_retail_commission_rate
    if retail_commission_rate > 0
      number_with_precision(retail_commission_rate, precision: 2)
    else
      ''
    end
  end
  ...
  def clean_values
    self.rent = 0 if rent.nil?

    self.service_commission_rate = 0 if self.service_commission_rate = nil

    self.retail_commission_rate = 0 if self.retail_commission_rate = nil
  end

  def pay_rent(date = Time.zone.now)
    payment = RentPayment.new
    payment.date = date
    payment.stylist_id = id
    payment.amount = rent
    payment.save
  end

  def rent_payments
    RentPayment.find_all_by_stylist_id(id)
  end
  ...
  def gross_service_sales(start_date, end_date)
    salon.earnings(start_date, end_date, id)['services'].to_f
  end

  def net_service_sales(start_date, end_date)
    gross_service_sales(start_date, end_date) * service_commission_rate
  end

  def gross_product_sales(start_date, end_date)
    salon.earnings(start_date, end_date, id)['products'].to_f
  end

  def net_product_sales(start_date, end_date)
    gross_product_sales(start_date, end_date) * retail_commission_rate
  end

  def self.everyone_stylist
    new(id: 0, name: 'All Stylists')
  end
